...
const { ACL_SUFFIX, META_SUFFIX } = require('../../src/server/defaults')
...
describe('HTTP APIs', function () {
    ...
    it('should have the same size of the file on disk', function (done) {
      server.get('/sampleContainer/solid.png')
        ...
        .end(function (err, res) {
          ...
          const size = fs.statSync(path.join(__dirname, '..', 'resources',
  ...
  describe('HEAD API', () => {
    it('should return content-type html by default', function (done) {
      ...
      function (done) {
        server.post('/sampleContainer/')
          ...
          .end(function (err) {
            ...
            const sizeTim = fs.statSync(path.join(__dirname, '..', 'resources',
              'timbl.jpg')).size
