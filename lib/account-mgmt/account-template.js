...
class AccountTemplate {
   ...
   */
  static for ({ userAccount, accountStore }) {
    const substitutions = AccountTemplate.templateSubstitutionsFor(userAccount)

    return new AccountTemplate({ substitutions, accountStore })
  }
  ...
  async provisionAccountFrom ({ templatePath, accountUrl }) {
    ...
    try {
      ...
      for (const filePath of accountTemplateFiles) {
        ...
        const target = new LdpTarget({ url: url.resolve(accountUrl, relativePath) })
        const resource = await accountStore.resource({ target })
        const contents = this.isTemplate(filePath)
          ? processHandlebarTemplate(fs.readFileSync(filePath, 'utf8'), this.substitutions)
          : fs.readFileSync(filePath)
        await accountStore.writeBlob({ resource, blob: contents })
      }
    } catch (error) {
      logger.error(error)
      throw error
