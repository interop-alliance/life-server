...
class AccountManager {
  ...
  constructor (options = {}) {
    ...
    this.accountStore = this.storage.accountStore
  ...
  async accountUrlExists (accountUrl) {
    ...
    return this.accountStore.exists({ target: rootAcl })
  }
  ...
  async provisionAccountStorage (userAccount) {
    ...
    const template = AccountTemplate.for({ userAccount, accountStore })
  ...
  async deleteAccountStorage (userAccount) {
    try {
      ...
      const userRootContainer = await this.accountStore.resource({ target })
      return this.accountStore.deleteContainer({ container: userRootContainer })
    } catch (error) {
  ...
  async loadAccountRecoveryEmail (userAccount) {
    ...
    const resource = this.accountStore.resource({ target })

    const rootAclGraph = await this.accountStore.loadParsedGraph({ resource })
