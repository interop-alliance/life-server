  ...
  async provisionAccountDid ({ userAccount }) {
    ...
    try {
      const { didDocument } = await generateDid({
        url: userAccount.accountUri
      })
      logger.info(`User DID generated: '${didDocument.id}'.`)

      // Save didDocument and keys
      const didDocLocation = new URL('/.well-known/did.json',
        userAccount.accountUri)
