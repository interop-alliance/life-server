...
class AccountManager {
   ...
   * @param [options.collections] {CollectionManager}
   */
  constructor (options = {}) {
    if (!options.host) {
   ...
   *
   *   ```
   *   let options = { host, multiuser, ldpStore }
   ...
   *
   * The reason it tests the root .acl for existence, and not the whole
   * directory, is to safeguard single-user-mode servers who initially just
   * point theirdata dir to an existing directory, and as a result cannot
   * register.
   *
  ...
  /**
   * Creates a user account storage directory (from a default account template).
   *
   * @param userAccount {UserAccount}
   *
   * @return {Promise}
   */
  async createAccountDir (userAccount) {
  ...
  createDefaultCollections (userAccount) {
    return this.collections.createDefaultCollections(userAccount)
  }
