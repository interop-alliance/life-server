...
const { generateDid } = require('../dids')
...
class AccountManager {
  ...
  async provisionAccountStorage (userAccount) {
    try {
      ...
      const template = AccountTemplate.for({ userAccount, accountStorage })
  ...
  async provisionAccountDid ({ userAccount }) {
    ...
    try {
      ...
      target = new LdpTarget({ url: didDocLocation.toString() })
      resource = await accountStorage.resource({ target })
      ...
      // Save user's keys
      const { exportKeys } = require('did-cli/lib/storage')
      const exportedKeys = await exportKeys(didKeys)

      const keysLocation = new URL(`/vault/keys/${did}.keys.json`, accountUri)
      target = new LdpTarget({ url: keysLocation.toString() })
      resource = await accountStorage.resource({ target })
      await accountStorage.writeBlob({
        resource,
