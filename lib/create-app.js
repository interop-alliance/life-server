...
function createApp (argv = {}) {
  ...
  // Init storage
  const storage = config.initStorage({ host, mapper })
...
/**
 * @param app.locals {object} Express.js app instance local variables hash
 * @param argv {object} Config options hashmap
 * @param argv.couchdb {object} DB config params (url, username, password)
 */
function initStorage ({app, argv}) {
  if (!argv.couchdb) { return }

  const couchUrl = new URL(argv.couchdb.url)
  couchUrl.username = argv.couchdb.username
  couchUrl.password = argv.couchdb.password
  const couch = couchClient(couchUrl.toString())
  app.locals.couch = couch
}
...
function initWebId (argv, app, storage) {
  ...
  let accountManager = AccountManager.from({
    ...
    collections: new CollectionManager({ couch: app.locals.couch })
  })
