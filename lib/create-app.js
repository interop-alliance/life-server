...
async function createApp (argv = {}) {
  ...
  const host = ServerHost.from(hostConfig)
  argv.host = host
  ...
  const storage = StorageManager.from({
    host, couchConfig: argv.couchdb, dbPath: argv.dbPath, saltRounds: argv.saltRounds
  })
...
async function initWebId (argv, app, storage) {
  ...
  app.post('/transaction', bodyParser.json(), TransactionRequest.post())
