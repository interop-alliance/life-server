...
function requestHandler ({host, store}) {
  ...
  return async function handleRequest (req, res, next) {
    try {
      ...
      const isUserHomeRequest = target.isRoot &&
        target.mediaType() === 'text/html' &&
        !target.url.startsWith(host.serverUri)

      if (isUserHomeRequest) {
        return HomeRequest.handle(req, res).catch(next)
      }

      const ldpStore = storage.storeForTarget({ target })
      const resource = await ldpStore.resource({ target })

      const request = await requestFrom({ req, target, resource, host, ldpStore })

      // check that request is permitted (throws error if not)
      const requiredPermissions = request.requiredPermissions

      const permissions = await wac.allow({
        resource, requiredPermissions, ldpStore, credentials: request.credentials
      })
 ...
 */
async function requestFrom ({ req, target, resource, host, ldpStore }) {
  const options = RequestOptions.from({ req, target, resource, host, ldpStore })
...
class RequestOptions {
   ...
   */
  static from ({ req, target, resource, host, ldpStore }) {
