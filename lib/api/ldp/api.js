...
class LdpHttpHandler {
  ...
  async handleRequest (req, res, next) {
    try {
      ...
      const resourceUri = this.host.parseTargetUrl(req)

      const acl = AclChecker.from({ resourceUri, req })

      // check that operation is permitted (throws error if not)
      // const permissions = await acl.allow(operation)
      const permissions = {}
  ...
  async operationFrom (req) {
    ...
    console.log('method:', options.method.toLowerCase())

    const Operation = LdpOperation.BY_METHOD[options.method.toLowerCase()]
    if (!Operation) {
