...
const { addLinks, addLink, Metadata } = require('../../header')
const debug = require('debug')('solid:ldp')
const { DEFAULT_RDF_TYPE, META_SUFFIX, ACL_SUFFIX } = require('../../constants')
...
class LdpRequest {
   ...
   */
  constructor ({ target, resource, headers, bodyStream, ldpStore, credentials = null, host, headerMeta }) {
    this.target = target
  ...
  writeCommonHeaders ({res, result = {}, permissions}) {
    ...
    const urlAcl = target.isAcl ? target.url : url.resolve(target.url, ACL_SUFFIX)
    const urlMeta = target.isMeta ? target.url : url.resolve(target.url, META_SUFFIX)

    // Add ACL and Meta Link in header
    addLink(res, urlAcl, 'acl')
    addLink(res, urlMeta, 'describedBy')
...
const PATCH_PARSERS = {
  'application/sparql-update': require('../../handlers/patch/sparql-update-parser'),
  'text/n3': require('../../handlers/patch/n3-patch-parser.js')
}
...
class LdpPatchRequest extends LdpRequest {
  ...
  async perform () {
    ...
    debug('PATCH -- Target <%s> (%s)', target.url, resource.contentType)
