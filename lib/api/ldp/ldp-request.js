...
class LdpRequest {
  ...
  static async putResource ({ ldpStore, resource, stream }) {
    ...
    try {
      await ldpStore.createResource({ resource, fromStream: stream })
    } catch (error) {
      // log error here
      throw error
    }
  ...
  async performMultipartUpload () {
    ...
    return new Promise((resolve, reject) => {
      busboy.on('file', (fieldName, fileStream, filename, encoding, contentType) => {
        ...
        ldpStore.newResource({ target, contentType })
          .then(resource => ldpStore.createResource({ resource, fromStream: fileStream }))
