...
const { parseMetadataFromHeader, addLinks } = require('../../header')
...
class LdpRequest {
   ...
   */
  constructor ({ target, resource, headers, bodyStream, ldpStore, credentials = null }) {
    this.target = target
...
class LdpPostRequest extends LdpRequest {
  ...
  async perform () {
    ...
    if (multipartUpload) {
      await this.handleMultipartUpload()
