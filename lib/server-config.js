...
function printDebugInfo (options) {
  ...
  if (options.couchdb) {
    debug.settings('CouchDB server: ' + options.couchdb.url)
  }
}
...
function initMapper (host) {
  return LegacyResourceMapper.from({ host })
}

/**
 * @param host {SolidHost}
 * @param mapper {LegacyResourceMapper}
 * @param couchdb {object} CouchDB config hashmap from argv
 *
 * @returns {StorageManager|*}
 */
function initStorage ({ host, mapper, couchConfig }) {
