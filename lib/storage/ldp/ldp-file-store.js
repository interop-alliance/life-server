...
class LdpFileStore extends LdpStore {
  ...
  /**
   * Creates a ReadStream for a given resource.
   *
   * @param resource {LdpFileResource}
   *
  ...
  async readContainerMeta ({ container }) {
    ...
    try {
      const metaPath = pathResolve(container.path, this.suffixMeta)
   ...
   * @returns {Promise<void>}
   */
  async writeBlob ({ resource = {}, path = resource.path, blob }) {
   ...
   */
  async writeGraph ({ resource, path = resource.path, graph, contentType = DEFAULT_RDF_TYPE }) {
    ...
    return this.writeBlob({ resource, path, blob: graphRdf, contentType })
  }
