...
async function allow ({ resource, requiredPermissions, ldpStore, credentials }) {
  ...
  try {
    ...
    hasAccess = await permissions.checkAccess(
    ...
    console.log('Error in allow():', error, 'user:', user)

    if (!user) {
      throw new HttpError(401, `Access to ${resource.target.url} requires authorization`)
    } else {
      throw new HttpError(403, `Access to ${resource.target.url} denied for ${user}`)
    }
  }
}
...
function aclUrlFor (url) {
  return isAcl(url) ? url : url + ACL_SUFFIX
}

function isAcl (url) {
  return url.endsWith(ACL_SUFFIX)
}
